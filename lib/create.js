...
async function handleFolderExist(projectName, cmd) {
  ...
  if (fs.existsSync(outdir)) {
    if (cmd.force) {
      ...
      isOverwrite ? (await fs.remove(outdir), chooseTemplate(projectName),successCallback(projectName)) : console.log("取消");
    }
  } else {
...
function downTemplate(template, outdir) {
  fs.copySync(`template/${template}`, path.join(process.cwd(), outdir));
}
/**项目拉取成功后提示
